...
export async function middleware(req: NextRequest) {
    ...
    // 2. Buyer dashboard routes — must be authenticated
    ...
    // 4. Product routes — KYC gate for buyers
    if (PRODUCT_ROUTES.some((r) => pathname.startsWith(r))) {
        if (user?.role === "admin") return NextResponse.next();
        if (user) {
            if (user.kycStatus !== "approved") {
                return isApi
                    ? NextResponse.json({ error: "KYC approval required" }, { status: 403 })
                    : NextResponse.redirect(new URL("/pending-approval", req.url));
            }
            return NextResponse.next();
        }
        // unauthenticated — let them see the page (will redirect on the client)
        return NextResponse.next();
    }
